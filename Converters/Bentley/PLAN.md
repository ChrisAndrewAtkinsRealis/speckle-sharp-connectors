...
**Last updated**: 2026-07-29 (fourth pass), added §8: recentering for
civil/GIS-scale absolute coordinates. Reported symptom: a georeferenced
"Bridge Assembly" DGN (real satellite-scale design coordinates) sent fine as
one coherent shape in MicroStation but rendered in the Speckle Viewer as
scattered, collapsed points instead of the bridge - the DGN source
(magenta-highlighted) is one continuous linear assembly; the Speckle Viewer
result was disconnected dots spread across the same rough footprint. See the
new row below.

**Last updated (third pass)**: 2026-07-28, fixing the §7 child-recovery gate
that was causing received `ExtendedElementsElement`/type-2 elements to come
back as a bounding-box mesh instead of their real shape — see the row below,
diagnosed directly from the `report.json`/`ELEMENT_COVERAGE.csv` evidence
gathered in the prior pass.
...
| 8 | Civil/GIS-scale coordinate precision (recentering) | ✅ Done, this pass | Root cause: every ToSpeckle raw coordinate converter shipped absolute native master-unit coordinates with zero recentering — confirmed by grepping this connector and finding no `GlobalOrigin`/`ReferencePoint`/`BasePoint` handling anywhere. For a georeferenced civil file (real state-plane-scale design coordinates, six-plus figures), that means mesh/curve vertices at magnitudes where WebGL's float32 vertex buffers lose the sub-meter precision needed to represent a small object's shape — it collapses to a point while its gross position survives, matching the reported "coherent bridge → scattered dots" symptom exactly. AutoCAD/Civil3D have a comparable `IReferencePointConverter`/`ReferencePointConverter` already (`Speckle.Converters.AutocadShared`), but it's opt-in (only recenters if the user manually sets a non-identity UCS before sending; the code's own comment flags it "needs to be revisited"). Added an automatic, scoped `IReferencePointConverter`/`ReferencePointConverter` for MicroStation instead (registered `AddScoped`, mirroring the AutoCAD DI pattern) — the first point converted in a send establishes the origin and every point (this one included) is returned relative to it, so nothing depends on the user setting anything up first. Wired into every ToSpeckle raw converter that produces absolute positions: `DPoint3dToSpeckleRawConverter`, `PolyfaceHeaderToSpeckleRawConverter` (mesh vertices — the direct cause of the reported `SolidX` "dots"), `PointListToSpecklePolylineRawConverter`, `MSBsplineCurveToSpeckleRawConverter`'s stroked display polyline, and `ElementToSpeckleFallbackConverter`'s range/bounding-box fallback mesh (so fallback objects stay aligned with everything else post-recentering). `DEllipse3dToSpeckleRawConverter`, `DPlane3dToSpeckleRawConverter`, and Surface control points are covered transitively since they all route through `DPoint3dToSpeckleRawConverter`'s injected `pointConverter`. Vectors (`DVector3dToSpeckleRawConverter`) are deliberately untouched — they're directions, not positions. `MicroStationRootObjectBuilder` records the resulting origin (`root["referencePointOrigin"]`/`root["referencePointOriginUnits"]`) after the send's conversion loop so a receiver can undo it. On receive, `MicroStationHostObjectBuilder.RestoreReferencePointOrigin` reads that back (converting units via `Speckle.Sdk.Common.Units.GetConversionFactor` in case the receiving file's master unit differs) and seeds the scope's converter via the new `SetOrigin`; `MicroStationUnitScaler.PointToNative`/`PointListToNative` add the origin back (converted to native UoRs) after scaling, so every ToHost geometry converter is covered automatically since they all already route through the scaler (confirmed: `LineToHostConverter`, `EllipseToHostConverter`, `ArcToHostConverter`, `CircleToHostConverter`, `PolylineToHostConverter`, `CurveToHostConverter`, `PointToHostConverter`, `MeshToHostConverter` — and `SolidXToHostConverter` delegates to the mesh converter). Added `ReferencePointConverterTests.cs` (origin capture, recentering, round-trip, explicit `SetOrigin`) and updated `FallbackConverterTests.cs` with a passthrough fake. Like the rest of this connector, unverified against a real Windows/MicroStation build — the Bentley SDK doesn't compile on Linux; needs a Windows pass sending/receiving an actual georeferenced file to confirm the Viewer renders one coherent shape instead of scattered points. |
